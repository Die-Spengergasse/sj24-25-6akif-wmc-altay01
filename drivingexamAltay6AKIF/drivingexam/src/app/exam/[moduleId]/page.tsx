...
export default function ExamPage() {
  ...
  useEffect(() => {
    ...
    getModules()
      ...
      .catch((err) => console.error('Error occured while loading the modules:', err))

    getExamQuestions(moduleId as string)
      .then((data) => dispatch({ type: 'load', payload: data }))
      .catch((err) => {
        console.error('Error occured while loading questions:', err)
        setError('Error while loading questions.')
      })
  ...
  if (!current) {
    return (
      <div className={styles.container}>
        ...
        <p><strong>Your Score:</strong> {state.totalPoints} / {state.totalReachable}</p>
  ...
  return (
    <div className={styles.container}>
      <h2 className={styles.heading}>Question {state.currentIndex + 1} von {state.questions.length}</h2>
      ...
      {!state.result && (
        <button onClick={handleSubmit} className={styles.checkBtn}>Checking Answer</button>
      )}

      {state.result && (
        <button onClick={handleNext} className={styles.nextBtn}>Next Question</button>
      )}
    </div>
  )
}
